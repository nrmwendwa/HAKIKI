...
const verdictConfig = {
  real: { label: "HALISI", icon: Shield, colorClass: "text-safe", bgClass: "bg-safe/10 border-safe/30", barClass: "bg-safe" },
  suspicious: { label: "INATIA SHAKA", icon: AlertTriangle, colorClass: "text-suspicious", bgClass: "bg-suspicious/10 border-suspicious/30", barClass: "bg-suspicious" },
  fake: { label: "FEKI", icon: ShieldX, colorClass: "text-destructive", bgClass: "bg-destructive/10 border-destructive/30", barClass: "bg-destructive" },
};
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.detail || "Uchambuzi umeshindwa. Tafadhali jaribu tena.");
      }
      
      const data = await res.json();
      setResult(data);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Uchambuzi umeshindwa. Tafadhali jaribu tena.";
      toast.error(message);
    } finally {
      setScanning(false);
    }
      ...
      <div className="container mx-auto max-w-3xl px-4">
        ...
        >
          <h1 className="font-display text-3xl font-bold sm:text-4xl">
            Skani <span className="text-gradient-primary">Picha</span>
          </h1>
          <p className="mt-3 text-muted-foreground">
            Pakia picha ili kuchunguza ikiwa ni halisi au imetengenezwa na AI.
          </p>
        </motion.div>
                  ...
                  >
                    ...
                    <div className="mt-6 flex gap-3">
                      <Button
                        variant="outline"
                        className="flex-1"
                        onClick={() => toast.success("Matokeo yameshirikiwa!")}
                      >
                      ...
                      <Button
                        variant="outline"
                        className="flex-1"
                        onClick={() => toast.info("Picha imeripotiwa kwa ukaguzi wa jamii.")}
                      >
